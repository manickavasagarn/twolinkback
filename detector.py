...
faceIDENTIFY = "model_2022_3/face-reidentification-retail-0095.xml" #faceIDENTIFY = "model_2022_3/facenet.xml"
...
def image_recognizer (frame, text, identity, face_point, threshold): 
    ...
    if identity.id != FaceIdentifier.UNKNOWN_ID:
        if (1-identity.distance) > threshold:
            ...
            textsize = cv2.getTextSize("Uknown", cv2.FONT_HERSHEY_SIMPLEX, 0.7, 1)[0]
            cv2.rectangle(frame, (xmin, ymin), (xmin + textsize[0], ymin- textsize[1]), (255, 255, 255), cv2.FILLED) 
            cv2.putText(frame, "Uknown", (xmin, ymin), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 0), 1)

testinstance = FrameProcessor()
metrics = PerformanceMetrics()
while True:
    start_time = perf_counter()
    ret, frame = camera.read()
    detections = testinstance.face_process (frame)
    frame = draw_face_detection (frame, testinstance, detections)
    metrics.update(start_time, frame)
    #cv2.imshow("face recognition demo", frame)
    #key = cv2.waitKey(1)
    #if key in {ord('q'), ord('Q'), 27}: camera.release()
